...
public static class ExtensionMethods {
	...
	public static T GetComponentInChildrenWithoutParent<T> ( this GameObject gameObject ) where T : Component {
		List<T> targets = new List<T> ();
		targets.AddRange ( gameObject.GetComponentsInChildren<T> () );

		for ( int i = 0 ; i < targets.Count ; i++ ) {
			if ( targets [i].transform == gameObject.transform ) {
				targets.RemoveAt ( i );
				i--;
			}
		}
	...
	public static Vector3 ToVector3XY(this Vector2 v, float? z = null ) {
		return new Vector3 ( v.x , v.y , z ?? 0f );
	}
