...
public class Toolbox : SingletonBehavior<Toolbox>, ISingletonBehavior {
	readonly static bool DEBUG_ENABLED = false;

    #region ISingletonBehavior
    bool ISingletonBehavior.SetDontDestroyOnLoadWhenSpawning => true;
    HideFlags ISingletonBehavior.HideFlagsToSetWhenSpawning => HideFlags.DontSave;
    #endregion
	...
	static public bool HasComponent<T> () where T : Component {
	...
	static public T FindComponent<T> () where T : Component {
		...
		{
			if ( DEBUG_ENABLED )
				Debug.LogFormat ( "[Toolbox] Found registered Component: {0}", component );
			return ( T ) component;
		}
		return null;
	}
	...
	static public T RegisterComponent<T> ( Component component ) where T : Component {
		...
		{
			if ( DEBUG_ENABLED )
				Debug.LogWarningFormat ( "[Toolbox] Cannot register another component of type {0}, because another one has already been registered beforehand", typeof ( T ) );
		} else // We are clear to add a new reference to the Component
		  {
			...
			if ( DEBUG_ENABLED )
				Debug.LogFormat ( "[Toolbox] Registering Component: {0}", component );
		}
		return ( T ) component;
	}
	...
	static public bool UnregisterComponent ( System.Type type ) {
		...
		if ( Instance.registeredComponents.TryGetValue ( type, out component ) ) // If we have a Component of that type registered
		{
			...
			if ( DEBUG_ENABLED )
				Debug.LogFormat ( "[Toolbox] Unregistering Component of type: {0}", type );
			return true;
		} else // No Component of that type registered
		  {
			if ( DEBUG_ENABLED )
				Debug.LogFormat ( "[Toolbox] Cannot unregister Component of type: {0}, because no Components of that type have been registered beforehand", type );
			return false;
		}
	}
